...
const navigation = [
  ...
  { name: 'Apps', href: '/apps', icon: FolderIcon, current: false },
  { name: 'SDM', href: '/sdm', icon: UsersIcon, current: false },
  { name: 'PAGES', href: '/pages', icon: DocumentDuplicateIcon, current: false }
];
...
const DashboardModule = () => {
  ...
  useEffect(() => {
    const fetchAppPackages = async () => {
      try {
        setLoading(true);
        const response = await fetch('/api/v1/apppackages');
        if (!response.ok) {
          throw new Error('Failed to fetch app packages');
        }
        const data = await response.json();
        setAppPackages(data);
      } catch (err) {
        setError(err.message);
        console.error('Error fetching app packages:', err);
      } finally {
        setLoading(false);
      }
    };
        ...
        <div className="sticky top-0 z-40 flex h-16 shrink-0 items-center gap-x-4 border-b border-gray-200 bg-white px-4 shadow-xs sm:gap-x-6 sm:px-6 lg:px-8 dark:border-white/10 dark:bg-gray-900 dark:shadow-none">
          ...
          <div className="flex flex-1 items-center gap-x-4 self-stretch lg:gap-x-6">
            ...
            <div className="flex items-center gap-x-4 lg:gap-x-6">
              <Link
                to="/pages"
                className="hidden text-sm font-semibold text-gray-500 transition hover:text-gray-900 lg:inline-flex dark:text-gray-400 dark:hover:text-white"
              >
                Pages
              </Link>
              <Link
              ...
              <>
                <div className="mb-8">
                  ...
                  <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
                    Welcome to your low-code application platform
                  </p>
                </div>

                <div className="grid grid-cols-1 gap-6 lg:grid-cols-2 xl:grid-cols-3">
                  <section className="rounded-2xl border border-indigo-100 bg-white p-6 shadow-sm transition hover:shadow-md dark:border-indigo-300/20 dark:bg-gray-900/80 dark:shadow-none">
                    <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Build experiences</h2>
                    <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
                      Launch new low-code applications using the visual designer and reusable components.
                    </p>
                    <Link
                      to="/apps"
                  ...
                  <section className="rounded-2xl border border-indigo-100 bg-white p-6 shadow-sm transition hover:shadow-md dark:border-indigo-300/20 dark:bg-gray-900/80 dark:shadow-none">
                    <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Automate processes</h2>
                    <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
                      Configure flows, triggers, and approvals that orchestrate work across your organization.
                    </p>
                    <button className="mt-4 inline-flex items-center gap-2 rounded-lg bg-violet-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-violet-500">
                      Open automation
                    </button>
                  </section>

                  <section className="rounded-2xl border border-indigo-100 bg-white p-6 shadow-sm transition hover:shadow-md lg:col-span-1 xl:col-span-1 dark:border-indigo-300/20 dark:bg-gray-900/80 dark:shadow-none">
                    <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Insights</h2>
                    <p className="mt-2 text-sm text-gray-600 dark:text-gray-400">
                      Track adoption metrics, usage analytics, and performance health of every app in the portfolio.
                    </p>
                    <div className="mt-6 grid grid-cols-2 gap-4">
                      <div className="rounded-xl bg-indigo-50 p-4 text-center dark:bg-indigo-500/10">
                        <p className="text-xs font-semibold uppercase tracking-wide text-indigo-600 dark:text-indigo-300">Total Apps</p>
                        <p className="mt-2 text-2xl font-bold text-indigo-700 dark:text-indigo-200">{appPackages.length}</p>
                      </div>
                      <div className="rounded-xl bg-indigo-50 p-4 text-center dark:bg-indigo-500/10">
                        <p className="text-xs font-semibold uppercase tracking-wide text-indigo-600 dark:text-indigo-300">Active Versions</p>
                        <p className="mt-2 text-2xl font-bold text-indigo-700 dark:text-indigo-200">
                          {appPackages.filter(a => a.version > 0).length}
                        </p>
                      </div>
                    </div>
                  </section>
                ...
                {appPackages.length > 0 && (
                  <div className="mt-8">
                    <div className="mb-4 flex items-center justify-between">
                      <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Recent Apps</h2>
                      <Link
                        to="/apps"
                    ...
                    <div className="grid grid-cols-1 gap-6 lg:grid-cols-2 xl:grid-cols-3">
                      {appPackages.slice(0, 6).map((app) => (
                        <section
                          key={app.id}
                          className="rounded-2xl border border-indigo-100 bg-white p-6 shadow-sm transition hover:shadow-md dark:border-indigo-300/20 dark:bg-gray-900/80 dark:shadow-none"
                        >
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <h3 className="text-lg font-semibold text-gray-900 dark:text-white">{app.appName}</h3>
                              {app.appDesc && (
                                <p className="mt-2 text-sm text-gray-600 dark:text-gray-400 line-clamp-2">{app.appDesc}</p>
                              )}
                              <div className="mt-4 flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400">
                                <span>Version {app.version}</span>
                                {app.createTime && (
                                  <span>{new Date(app.createTime).toLocaleDateString()}</span>
                                )}
                              </div>
                            </div>
                            {app.thumnail && (
                              <img
                                src={app.thumnail}
                                alt={app.appName}
                                className="ml-4 size-16 rounded-lg object-cover"
                              />
                            )}
                          </div>
                          <div className="mt-6 flex gap-2">
                            <button className="flex-1 rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-indigo-500">
                              Open
                            </button>
                            <button className="rounded-lg border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 transition hover:bg-gray-50 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700">
